...
*   **Disaster Recovery**: ZIP export with manifest.json + attachment binaries via `/api/admin/export.zip` (sync) or background jobs via `/api/admin/export-jobs/zip` (async). Restore CLI at `scripts/restore-org.ts` supports dry-run/apply modes with ID remapping for restoring into new organisations. Background jobs worker polls every 2500ms with concurrency guard.
