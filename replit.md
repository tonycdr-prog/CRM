...
*   **User Experience**: Touch-optimized interface, keyboard navigation for data entry, offline status indicator, signature capture, and auto-save indicator. PWA installability with home screen install, offline shell, and service worker caching.
*   **Camera & Image Handling**: Camera capture with automatic image compression (max 1600px, 75% quality JPEG) to reduce upload bandwidth while preserving file metadata. Preview modal with retake option before upload confirmation.
...
*   **Synchronization**: Offline delta sync with a change queue system that tracks modifications and syncs to the server upon connectivity restoration. Background sync with progress bar showing pending items and completion percentage. Auto-retry on reconnection.
*   **Disaster Recovery**: ZIP export with manifest.json + attachment binaries via `/api/admin/export.zip` (sync) or background jobs via `/api/admin/export-jobs/zip` (async). Server-side import via `POST /api/admin/import` supports dry_run mode (validation only) or apply mode (writes data with ID remapping). Import remaps IDs for templates, entities, entityRows, inspections, inspectionResponses, files, and attachments. Imports are logged in `import_runs` table for audit purposes. Restore CLI at `scripts/restore-org.ts` supports dry-run/apply modes with ID remapping for restoring into new organisations. Background jobs worker polls every 2500ms with concurrency guard.
*   **Security Architecture**: Session-based tenancy enforcement via Replit Auth OIDC. All API routes use an authenticated router pattern (`apiRouter`) mounted with `isAuthenticated` middleware. UserId is extracted from session (`req.user.claims.sub`) rather than client-provided parameters. Public routes (portal, auth, downloads) are explicitly exempted. Server utilities in `server/utils/routeHelpers.ts` provide `asyncHandler`, `getUserId`, and validation helpers. **Tenant Hardening (Phase 0)**: Jobs table includes `organizationId` column with NOT NULL constraint and indexes (`jobs_org_idx`, `jobs_org_created_idx`). All job CRUD operations filter by organizationId using org-scoped storage methods (`getJobsByOrg`, `getJobByOrg`, `updateJobByOrg`, `deleteJobByOrg`). Access helpers in `server/lib/access.ts` provide `getOrgId`, `getUserId`, and `assertJobAccess` functions.
