...
test("smoke control module constants are defined", () => {
  assert.strictEqual(MODULES.LIFE_SAFETY, "life-safety");
  assert.ok(MODULE_LABELS[MODULES.LIFE_SAFETY]);
  assert.ok(MODULE_TAGLINES[MODULES.LIFE_SAFETY]?.includes("Smoke Control"));
});

test("module nav exposes Smoke Control links", () => {
  const nav = MODULE_NAV[MODULES.LIFE_SAFETY];
  assert.ok(nav, "Smoke Control nav should be present");
...
test("modules list contains Smoke Control entry", () => {
...
test("module definitions include ownership metadata", () => {
  const smokeControl = getModuleList().find((m) => m.id === MODULES.LIFE_SAFETY);
  assert.ok(smokeControl, "Smoke Control module should exist");
  assert.strictEqual(smokeControl?.key, MODULES.LIFE_SAFETY);
  assert.ok(smokeControl?.ownsRoutes.length);
  assert.ok(Array.isArray(smokeControl?.ownsWidgets));
  assert.ok(Array.isArray(smokeControl?.ownsSidebarSections));
});
